...
public interface TaskExecutionLogRepository extends MongoRepository<TaskExecutionLogEntity, String> {
    ...
    @Query("{ 'taskId': { $in: ?0 }, 'status': ?1, 'applicationId': ?2, 'updatedAt': { $gt: ?3, $lte: ?4 } }")
    List<TaskExecutionLogEntity> findTasksByTaskIdsStatusAndApplicationIdWithinTimeRange(
            Set<String> taskIds, String status, String applicationId, Date initiatedAt, Date endTime);
}